...
async function loadAlerts() {
  try {
    ...
    document.getElementById('ticker-content').innerHTML =
      '<span class="ticker-item"><i class="fas fa-om"></i> Welcome to Sacred Places Hub - AI-Powered Pilgrim Guide</span>' +
      '<span class="ticker-item"><i class="fas fa-om"></i> Welcome to Sacred Places Hub - AI-Powered Pilgrim Guide</span>';
    document.getElementById('panchang-grid').innerHTML = '<p style="color:var(--text-muted);text-align:center;grid-column:1/-1"><i class="fas fa-info-circle"></i> Panchang data will load when API is available</p>';
  }
}
...
async function loadTempleOfDay() {
  try {
    ...
    if (t.thumbnail) {
      ...
      const iconEl = imgEl.querySelector('i');
      if (iconEl) iconEl.style.display = 'none';
    }

    content.innerHTML = `
      <h3>${escapeHtml(t.name || 'Sacred Place of the Day')}</h3>
      <div class="tod-tagline">"${escapeHtml(t.tagline || '')}"</div>
      <div class="tod-meta">
        <span>${religionIcon(t.religion)} ${escapeHtml(t.deity || '')}</span>
    ...
    document.getElementById('tod-content').innerHTML = '<p style="color:var(--text-muted)"><i class="fas fa-info-circle"></i> Sacred Place of the Day will load when API is available. Try refreshing in a minute.</p>';
  }
}
...
async function loadMantra() {
  ...
  try {
    ...
    el.innerHTML = `
      ...
      ${m.deity ? `<div class="mantra-deity">${religionIcon(m.religion)} ${escapeHtml(m.deity)} ${m.chantCount ? '&bull; Chant ' + escapeHtml(m.chantCount) + ' times' : ''}</div>` : ''}
    `;
  } catch {
    el.innerHTML = '<div class="mantra-sanskrit">Om Namah Shivaya</div><div class="mantra-meaning">I bow to Lord Shiva — Prayer of the day loads when API is available</div>';
  }
}
...
async function loadDidYouKnow() {
  ...
  try {
    ...
    if (facts.length) {
      ...
      el.innerHTML = '<p style="color:var(--text-muted)"><i class="fas fa-lightbulb"></i> Fascinating facts will load when API is available.</p>';
    }
  } catch {
    el.innerHTML = '<p style="color:var(--text-muted)"><i class="fas fa-lightbulb"></i> Fascinating facts will load when API is available.</p>';
  }
...
async function loadHoroscope(rashi, cardEl) {
  ...
  try {
    ...
    if (d.error || !d.prediction) {
      const errMsg = d.error ? (d.error.includes('Rate limit') ? 'API rate limit reached. Please wait a few minutes and try again.' : d.error) : 'Could not generate horoscope. Please try again.';
      resultEl.innerHTML = `
        <div class="horoscope-header">
          <div class="rashi-icon">${symbol}</div>
          <div><h3>${escapeHtml(rashi)}</h3><div class="date"><i class="fas fa-calendar-alt"></i> ${escapeHtml(dateParam)}</div></div>
        </div>
        <p style="padding:1rem;color:var(--text-muted);text-align:center"><i class="fas fa-exclamation-circle" style="color:var(--accent);margin-right:0.3rem"></i>${escapeHtml(errMsg)}</p>
        <button class="btn btn-outline" style="margin:0 auto;display:block" onclick="loadHoroscope('${escapeHtml(rashi).replace(/'/g, "\\'")}', document.querySelector('.rashi-card.active'))"><i class="fas fa-redo"></i> Retry</button>`;
      return;
    }
    ...
    resultEl.innerHTML = `
      ...
      <p style="padding:1rem;color:var(--text-muted);text-align:center"><i class="fas fa-exclamation-circle" style="color:var(--accent);margin-right:0.3rem"></i>Error loading horoscope. Please try again.</p>
      <button class="btn btn-outline" style="margin:0 auto;display:block" onclick="loadHoroscope('${escapeHtml(rashi).replace(/'/g, "\\'")}', document.querySelector('.rashi-card.active'))"><i class="fas fa-redo"></i> Retry</button>`;
  }
}
